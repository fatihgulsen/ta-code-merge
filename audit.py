...
def extract_core_tokens(name):
    """Extract distinctive tokens (exclude legal suffixes, geo words)"""
    generic = {
        ...
        'COMMUNICATIONS', 'COMMUNICATIONS.', 'OPERATIONS', 'SYSTEMS', 'SERVICES',
...
def audit_match(master, variant, leak_flag):
    ...
    # Same core tokens (allows for reordering, extra legal suffixes)?
    # For Mexican firms, they differ in: spacing, punctuation, abbreviations of legal suffixes
    m_set = set(m_tokens)
    v_set = set(v_tokens)
    
    if m_set == v_set:
        # Same distinctive tokens -> CORRECT
        return "CORRECT", "variant_ok"
    
    # Subset: one is substring of the other (truncation/incomplete)?
    # "GRUPO COMERCIAL RUBIN & SAINZ S.A. D" vs "GRUPO COMERCIAL RUBIN & SAINZ"
    # Both have same core "GRUPO COMERCIAL RUBIN SAINZ"
...
for input_path, batch_name in files:
    ...
    with open(input_path) as inf, open(output_path, 'w', encoding='utf-8') as outf:
        for line in inf:
            if not line.strip():
                continue
